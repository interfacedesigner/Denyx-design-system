...
3. **옵션바** (선택) — `OptionbarInstanceSelector` + `LiveTimerCompact` + `PresetSelect` 등을 직접 조합
...
> 상세 페이지를 만들어 줘.
> - 파일: `pages/{제품}/{페이지명}.tsx`
> - 뼈대: `DashboardLayout` + `PageHeader` (제목: '{대상} 상세')
...
| 만들 화면 | 프리셋 | 핵심 부품 |
|---|---|---|
| 지표 모니터링 대시보드 | **A. 기본 대시보드** | PageHeader + DataTable |
